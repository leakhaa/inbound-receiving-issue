...
def send_ai_email_with_screenshot(user_email: str, subject: str, context: str, details: dict, screenshot_data: str = None, html_format: bool = False):
    ...
    """
    # Update details with screenshot information
    details_with_screenshot = details.copy()
    details_with_screenshot['screenshot_provided'] = bool(screenshot_data)
    
    # Generate AI content with updated details
    ai_content = generate_ai_email_content(context, details_with_screenshot, user_email)
    
    # Add HTML formatting if requested
    if html_format and details.get('html'):
        ai_content += f"<br><br>{details['html']}"
    
    # Send email with AI-generated content
    send_email_with_screenshot(user_email, subject, ai_content, screenshot_data, html_format)
...
def resolve_issue(scenario: str, params:dict, user_email:str, screenshot_data:str = None)-> str:
    ...
    if scenario == "missing_asn":
        ...
        if check_asn_exists(asn) and summary["po_pallets"] == summary["asn_pallets"] and summary["po_qty"] == summary["asn_qty"]:
            ...
            if status == "triggered":
                if check_asn_exists(asn) and summary["po_pallets"] == summary["asn_pallets"] and summary["po_qty"] == summary["asn_qty"]:
                    ...
                    send_ai_email_with_screenshot(user_email, "ASN Trigger Failed", "ASN Trigger Failed", {"asn_id": asn}, screenshot_data)
        ...
        path_or_df = wait_for_excel_from_sap("mismatch")

# If it returns a path string:
        if isinstance(path_or_df, str) and path_or_df:
            df = parse_excel_to_df(path_or_df)
            live_print("Excel path received: " + path_or_df)
# If it already returns a DataFrame:
        elif isinstance(path_or_df, pd.DataFrame) and not path_or_df.empty:
            df = path_or_df
            live_print("Excel DataFrame received, shape: " + str(path_or_df.shape))
        else:
            live_print("No Excel received or empty DataFrame returned by SAP.")
            live_print("path_or_df is empty or None")
            # Step 3: Extract PO from Excel if not originally provided
        df['asn_id'] = df['asn_id'].apply(lambda x: str(x).zfill(5))
        df['po_id'] = df['po_id'].apply(lambda x: str(x).zfill(10))
        df['pallet_id'] = df['pallet_id'].apply(lambda x: str(x).zfill(15))
...
# --- NEW LOGIC: Collect missing entries ---
        ...
        missing_asn_entries = []
        for asn_value in df["asn_id"].unique():
            live_print(f"Checking ASN {asn_value} in the system")
            if not check_asn_exists(asn_value):
                missing_asn_entries.append(asn_value)

# Collect missing PO entries from entire Excel
        missing_po_list = []
        for po_value in df["po_id"].unique():
            live_print(f"Checking PO {po_value} in the system")
            if not check_po_exists(po_value):
                 missing_po_list.append(po_value)

# Collect missing pallet entries from entire Excel
        existing_pallet_ids = get_existing_pallet_ids()
        missing_pallets = df[~df["pallet_id"].isin(existing_pallet_ids)]["pallet_id"].unique().tolist()
