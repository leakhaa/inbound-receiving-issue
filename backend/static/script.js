class Chatbot {
    ...
    async sendMessage() {
        ...
        try {
            ...
            // Render any server-side logs
            if (data.print_outputs && Array.isArray(data.print_outputs) && data.print_outputs.length > 0) {
            ...
            if (data.end) {
                this.disableInput();
                return; // Do not re-enable input in finally
            }
            ...
            this.hideLoading();
            // Only enable input if session not ended
